var Subtyperooms = {
		...
		Refresh: function(el) {	
			...
			$.ajax({
				type: "POST",
				async: true,
				url: '/subtyperooms/refresh',
				dataType: 'html',
				success:function(result){
					//$('#datatable-responsive_wrapper').remove();
					$('#subtyperooms-table').html(result);
		...
		Delete: function (el) {
			var id = $(el).attr("subtyperoomid");
			console.log(id);
			App.loadAjax();

			$.ajax({
				url: '/subtyperooms/delete',
				type: 'get',
				data: {
					id: id
				},
							...
							Yes: {
								...
								click: function() {
									...
									$.ajax({
										...
										success: function (result) {
											Subtyperooms.Refresh(el);
											App.loadAjax('unmask');
									   		$('#delete-subtyperoom').remove();
										}
									});
								}
							},
